        ...
        <div class=" bg-white table__box">
            <table class="table">
                ...
                <tr role="row">
                    ...
                    <td>
                        <a href="{{ route('comments.destroy', $comment->id) }}" class="item-delete mlg-15" onclick="destroyComment(event, {{ $comment->id }})" title="حذف">حذف </a>
                        @if($comment->is_approved)
                        <a href="{{ route('comments.update', $comment->id) }} " onclick="updateComment(event, {{ $comment->id }})"  class="item-reject mlg-15" title="رد"> رد</a>
                        @else
                        <a href="{{ route('comments.update', $comment->id) }} " onclick="updateComment(event, {{ $comment->id }})" class="item-confirm mlg-15" title="تایید">تایید</a>
                        @endif
